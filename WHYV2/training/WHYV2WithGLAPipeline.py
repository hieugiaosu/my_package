...
class WHYV2GLAPipelineWithWham(TrainingPipeline):
    def __init__(
            self, 
            model: Module, 
            train_dataset, 
    ...
    def train_iter(self,epoch,start_time):
        ...
        for data in self.train_loader:
            ...
            ref = data['ref']
            audio_len = src0.shape[-1]
            if self.wham_train is not None:
                mix = self.wham_train.add_noise_batch(mix)
            mix = torch.cat([ref,mix],dim=-1)
            mix = mix.to(self.device)
            num_batch += 1
            with autocast(enabled=False):  # Use autocast for mixed precision
                if epoch <= self.warm_up:
                    yHat = self.model(mix,emb0, return_hidden = False)
                    loss = self.si_sdr_fn(yHat[:,-audio_len:],src0.squeeze())
                    if loss.isnan().detach().item():
                        print(yHat)
                        raise ValueError("loss is nan")
                else:
                    yHat = self.model(mix, emb0, return_hidden = False)
                    si_sdr = self.si_sdr_fn(yHat[:,-audio_len:], src0)
                    mix_constraint = self.mixture_constraint_fn(yHat[:,-audio_len:], src0)
    ...
    def validate_iter(self):
        ...
        with torch.no_grad():
            for data in self.val_loader:
                mix = data['mix']
                src0 = data['src0'].to(self.device).squeeze()
                emb0 = data['emb0'].to(self.device)
                ref = data['ref']
                audio_len = src0.shape[-1]
                ...
                with autocast(enabled=False):  # Use autocast for mixed precision
                    yHat = self.model(mix, emb0, return_hidden=False)
                    loss = self.si_sdr_fn(yHat[:,-audio_len:], src0)
                tot_loss += loss.cpu().detach().item()
                del mix, src0, yHat, loss, emb0
                torch.cuda.empty_cache()
                gc.collect()
