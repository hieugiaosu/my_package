...
class DistillPipelineWithWham(TrainingPipeline):
    def __init__(
            self, 
            model: Module,
    ...
    def train_iter(self,epoch,start_time):
        ...
        for data in self.train_loader:
            ...
            with autocast():
                with torch.no_grad():
                    teacher_features = self.teacher_model(mix,emb0,return_distill_feature=True,is_teacher=True)
            num_batch += 1
            with autocast():  # Use autocast for mixed precision
                if epoch <= self.warm_up:
                    yHat = self.model(mix,emb0,return_distill_feature=True)
                    si_sdr = self.si_sdr_fn(yHat['output'],src0)
                    encoder_feature_loss = self.mse(
                        yHat['encoder_feature'],teacher_features['encoder_feature']
                        )
                    tf_gridnet_block_feature_loss = self.mse(
                            yHat['tf_gridnet_block_feature'],teacher_features['tf_gridnet_block_feature']
                        )
                    loss = si_sdr + encoder_feature_loss + tf_gridnet_block_feature_loss
                else:
                    ...
                    loss = si_sdr + mix_constraint + encoder_feature_loss + tf_gridnet_block_feature_loss
            ...
            if epoch <= self.warm_up:
                ...
                del mix, src0, yHat, loss, emb0, teacher_features, encoder_feature_loss, tf_gridnet_block_feature_loss
            else:
                print(f"--------------batch:{num_batch}/{total_batch}---------loss:{loss.cpu().detach().item()}|si-sdr:{si_sdr.cpu().detach().item()}----------")
                del mix, src0, yHat, loss, si_sdr, mix_constraint, emb0, teacher_features, encoder_feature_loss, tf_gridnet_block_feature_loss
    ...
    def validate_iter(self):
        ...
        with torch.no_grad():
            for data in self.val_loader:
                ...
                num_batch += 1
                with autocast():  # Use autocast for mixed precision
                    yHat = self.model(mix, emb0,return_distill_feature=False)
