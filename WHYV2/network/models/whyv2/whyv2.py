...
class WHYV2_version2(nn.Module):
    def __init__(
            self,
            n_fft=128,
            hop_length=64,
    ...
    def forward(self, input, speaker_embedding, hidden: List[CausalWHYV2BlockHidden] = [], return_hidden=False):
        audio_length = input.shape[-1]

        if len(hidden) == 0:
            hidden = [dict({"lstm_h":None,"attention_h":None}) for _ in range(self.n_layers)]
        x = input
        ...
        for i in range(self.n_layers):
            ...
            x, hidden_i = self.whyv2_block[i](x,gtf,gtb,hidden[i],return_hidden=True)
