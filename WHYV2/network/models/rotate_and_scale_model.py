...
from ..modules.condition.scale_and_rotate import ScaleAndRotateCondition
...
class WHYVwithRotateCondition(nn.Module):
    def __init__(
            self,
            n_fft=128,
            hop_length=64,
        ...
        self.gates = nn.ModuleList(
            [
                ScaleAndRotateCondition(emb_dim,conditional_dim,n_freqs=n_freqs)
                for _ in range(n_layers)
            ]
        )
